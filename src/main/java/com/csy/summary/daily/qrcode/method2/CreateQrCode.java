...
public class CreateQrCode {
    ...
    public CreateQrCode(Color borderColor, int logoPart) {
     ...
     * @param createQrCode 生成的二维码
     * @return boolean
     * @author XinLau
     * @since 2019年2月12日下午5:23:11
     */
    public static boolean addLogo(File qrPic, File logoPic, CreateQrCode createQrCode) {
        try {
            ...
            // 保持二维码是正方形的
            int widthLogo = image.getWidth() / createQrCode.getLogoPart();
            int heightLogo = image.getWidth() / createQrCode.getLogoPart();
            // 计算图片放置位置
            ...
            g.setStroke(new BasicStroke(createQrCode.getBorder()));
            g.setColor(createQrCode.getBorderColor());
    ...
    public boolean createLogoQrCode(String qrcPath, String content, int width, int height, String logoPath) {
        ...
        if (createQrCode(qrcPath, content, width, height)) {
            ...
            // 判断logo是否添加成功
            if (!addLogo(qrcFile, logoFile, createQrCode)) {
                return false;
            }
            return true;
        } else {
    ...
    public boolean createWordQrcode(String newImageWithText, String targetImage, String text, int width, int height) {
        ...
        if (createQrCode(newImageWithText, text, width, height)) {
            ...
            if (org.apache.commons.lang3.StringUtils.isNotBlank(targetImage)) {
                // 二维码图片添加文字是否成功
                return pressText(text, newImageWithText, targetImage, fontStyle, Color.red, font, width, height);
            } else {
                // 二维码图片添加文字是否成功
                return pressText(text, newImageWithText, newImageWithText, fontStyle, Color.red, font, width, height);
            }
        }
        return true;
    }
